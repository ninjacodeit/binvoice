...
namespace NinjaCode\BInvoice;
use NinjaCode\BInvoice\Lib\Rotation;


class Invoice extends Rotation
{
    ...
    function __construct($size='A4',$currency='zł',$language='pl')
    {
        $this->columns = 5;
        $this->items = array();
        $this->totals = array();
        ...
        $this->AddFont('Arial','','Arial.php');  //dodaje swoją czcionkę arialpl do dokumentu
        $this->SetFont('Arial','', 12);  //ustawia czcionkę arialpl, rozmiar 12
        $this->AliasNbPages();
    ...
    function Header()
    {
        ...
        $this->Cell(0,5,iconv("UTF-8", "ISO-8859-2",strtoupper($this->title)),0,1,'R');
        ...
        $this->Cell(32,$lineheight,iconv("UTF-8", "ISO-8859-2",strtoupper($this->l['number']).':'),0,0,'L');
...
//		$this->Cell(32,$lineheight,iconv("UTF-8", "ISO-8859-2", strtoupper(iconv("UTF-8", "iso-8859-1",$this->l['date']))).':',0,0,'L');
        ...
        if($this->PageNo()==1)
        {
            ...
            for($i=1; $i<max(count($this->from),count($this->to)); $i++) {
                $this->Cell($width,$lineheight,iconv("UTF-8", "ISO-8859-2",$this->from[$i]),0,0,'L');
                $this->Cell(0,$lineheight,iconv("UTF-8", "ISO-8859-2",$this->to[$i]),0,0,'L');
                $this->Ln(5);
            }
            $this->Ln(-6);
        }
        ...
        if(!isset($this->productsEnded))
        {
            ...
            $this->Cell($this->firstColumnWidth,10,iconv("UTF-8", "ISO-8859-2",strtoupper($this->l['product'])),0,0,'L',0);
            $this->Cell($this->columnSpacing,10,'',0,0,'L',0);
            $this->Cell($width_other,10,iconv("UTF-8", "ISO-8859-2",strtoupper($this->l['amount'])),0,0,'C',0);
            $this->Cell($this->columnSpacing,10,'',0,0,'L',0);
            $this->Cell($width_other,10,iconv("UTF-8", "ISO-8859-2",strtoupper($this->l['vat'])),0,0,'C',0);
            $this->Cell($this->columnSpacing,10,'',0,0,'L',0);
            $this->Cell($width_other,10,iconv("UTF-8", "ISO-8859-2",strtoupper($this->l['price'])),0,0,'C',0);
            if(isset($this->discountField))
            {
                $this->Cell($this->columnSpacing,10,'',0,0,'L',0);
                $this->Cell($width_other,10,iconv("UTF-8", "ISO-8859-2",strtoupper($this->l['discount'])),0,0,'C',0);
            }
            $this->Cell($this->columnSpacing,10,'',0,0,'L',0);
            $this->Cell($width_other,10,iconv("UTF-8", "ISO-8859-2",strtoupper($this->l['total'])),0,0,'C',0);
            $this->Ln();
            $this->SetLineWidth(0.3);
            $this->SetDrawColor($this->color[0],$this->color[1],$this->color[2]);
    ...
    function Body()
    {
        ...
        if($this->items) {
            foreach($this->items as $item)
            {
                if($item['description'])
                {
                    ...
                    $calculateHeight->MultiCell($this->firstColumnWidth,3,iconv("UTF-8", "ISO-8859-2",$item['description']),0,'L',1);
                ...
                $x = $this->GetX();
                $this->Cell($this->firstColumnWidth,$cHeight,iconv("UTF-8", "ISO-8859-2",$item['item']),0,0,'L',1);
                if($item['description'])
                {
                    $resetX = $this->GetX();
                    $resetY = $this->GetY();
                    ...
                    $this->MultiCell($this->firstColumnWidth,3,iconv("UTF-8", "ISO-8859-2",$item['description']),0,'L',1);
                    //Calculate Height
                    $newY = $this->GetY();
        ...
        foreach($this->addText as $text)
        {
            if($text[0] == 'title')
            {
                $this->SetFont($this->font,'',9);
                $this->SetTextColor(50,50,50);
                $this->Cell(0,10,iconv("UTF-8", "ISO-8859-2",strtoupper($text[1])),0,0,'L',0);
                $this->Ln();
                $this->SetLineWidth(0.3);
                $this->SetDrawColor($this->color[0],$this->color[1],$this->color[2]);
                $this->Line($this->margins['l'], $this->GetY(),$this->document['w']-$this->margins['r'], $this->GetY());
            ...
            if($text[0] == 'paragraph')
            {
                ...
                $this->SetFont($this->font,'',8);
                $this->MultiCell(0,4,iconv("UTF-8", "ISO-8859-2",$text[1]),0,'L',0);
                $this->Ln(4);
            }
        }
    }
